...
resources to achieve set goal). However, such a model is completely
insufficient for formalization of the task in order to find the most effective
conceptual solutions if there is no standard solution to the
problem. Consequently, the SMART model does not open up the possibility of
applying TRIZ methods; therefore, it does not can be considered as a method of
preliminary formalization of inventive situation. In fact, the use of the
SMART model does not allow the solver to switch from inventive situation to
the task (more precisely, to the system of tasks) suitable for further
processing using TRIZ tools. \textbf{According to the author, the use of TRIZ
  for the solution of organizational and managerial tasks in the first place
  makes it difficult it is the lack of reliable and relatively simple tools,
  allowing to move from an inventive situation to a system of tasks in the
  field of business systems}.
...
one of the most important competencies modern manager. However, to formalize
the inventive situation in organizational and managerial tasks, this generally
accepted method does not give the required result - it does not help to
isolate a system of tasks from an inventive situation, which can subsequently
be processed with TRIZ tools.
...
system of particular tasks to its elements (subsystems and supersystems).
Otherwise, we get a contradiction that is extremely general nature, and
therefore, there is a high risk of getting output commonplace decisions. The
author recommends formulating contradictions to each of particular tasks posed
after the analysis of the IFS. Otherwise, the solver risks too “narrow“ the
obtained solutions, as a result, a situation is possible when the goal the
solver will not be finally reached [10], [11].
...
solver believes that an organized social system does not function rationally,
which means, unlike from the previous method of setting tasks, has a narrower
scope. how as a rule, this tool is used to analyze the inventive situation in
in order to increase the efficiency of standard, well-established processes in
organized social systems.

The technology for describing business processes in world practice is not bad
standardized and described by generally accepted notations, e.g. IDEF0, BPMN
and along with others [34]. In TRIZ, a similar method is also actively used in
the form of \emph{flow analysis}. And although the descriptions of business
processes and the analysis of flows are not quite the same thing [11] (flow
analysis describes the transfer of matter, energy or information, has sources,
consumers and the flow path, and the description of the business process
focuses on the list of operations performed by the process owner), yet these
...
Most often, TRIZ practitioners use the system operator for this purpose [25].
Using the system operator, you can see the development of the system in
dynamics and to predict its structure in the future, and also to consider the
structure a functioning system - describe the supersystems and subsystems of
the studied system (...  sub-subsystems, subsystems, system, super-systems,
super-super-systems ...).

True, the system operator has a significant drawback - he is looking at the
development system through the composition of its elements, but does not take
into account the layers of the system and group elements, which, of course,
limits the capabilities of this tool in terms of descriptions of organized
social systems. In addition, the structure of functioning the system is taken
only element-wise, in contrast to the schematization that looks through the
layers, groups, communications, processes and functions. And if the
supersystems are on completely different management layers, what to do in this
case? How to set specific tasks for effectiveness of managing system elements?
System Management Phenomenon the operator does not reflect at all, it
describes the structure of the system as if “whatever” (fig. 3).
...
supersystems, the impossibility of drawing relationships between elements of
the system and supersystems (the structure is taken as if in isolation,
without indicating the connections between the elements of the system), as
well as the impossibility of demonstrating in the model the control circuit,
which is critical moment when studying the inventive situation with the aim
...
\textbf{In contrast to the specific inventive situation posed by the customer
  in organized social system, tasks that should be studied using system
  operator, are usually strategic in nature.  For example, the author carried
  out a project to develop a methodology for describing business processes for
  the company in such a way that the development dynamics of the company are
  not inhibited, processes could change as fast as the situation
  requires. Needed to find conceptual solution. To solve the problem, the main
  tool The system operator was taken for analysis ( see Appendix 2 ), since
  the task was strategic nature and situational interaction of system elements
...
relationship between the elements of the system, functions, and modern types
of PSA allow you to show groups of elements and even take into account the
processes occurring in the system [17]. Functional Example shown in fig. 4:
...
subsequently can be solved using TRIZ tools. However, these methods have all
the same key flaw characteristic of any methods developed for analysis of
technical systems.  All these methods do not take into account the control
context between system elements, but also do not take into account that part
of the elements of a functioning system are good modifiable objects of the
material world, and some are subjects, that is, people, performing certain
functions, but with their own goals, dynamically changing emotions, patterns
of behavior, etc., that is, objects, hard predictable behavior. Describing
such objects requires have a special language in order to develop solutions
that can later be replicate.
...
\textbf{The author believes that functional analysis has great potential for
  solving organizational and managerial tasks, in the author’s book [11] an
  example is examined in detail solving a similar problem, but this tool is
  more likely to apply to a certain class of organizational and managerial
  tasks related to the optimization of organized social systems and their
  subsystems [39] and is not quite convenient for preliminary analysis
...
\section[Schematization at MMK]{Schematization developed at the Moscow
  ...
  and its application for preliminary inventive analysis the situation}
...
similar to the main tools of military strategists - a map, the detail of which
is manifested gradually, in the course of the thought activity of the officers
planning the military operation. The author considers these developments to be
an excellent alternative to the SMART method with the point of view of
formalizing the inventive situation, but in its purest form getting synthesis
schematization + TRIZ was not so easy.

In essence, schematization is a visualization of an inventive situation made
by in accordance with the \textbf{categories of the system} and understanding
of the functioning of the system [10]. It is this tool that the author
considers optimal for the primary analysis of the inventive situation when
solving organizational and managerial tasks. It should be noted that the
author had to restore the following categories of schematization
independently, based on a detailed study of the works of G.P.  Schedrovitsky,
as currently schematization is greatly simplified and many SMD methodologists
build schemes without taking into account the following categories of systems,
...
Defining a framework on the example of the organizational and managerial task
“How introduce the technology of sales of complex solutions, overcoming
resistance from sales managers? ”From the task it’s clear what needs to be
overcome resistance of sales staff to the implementation of the new system.
...
mean that the tasks in the field of marketing and sales - these are
organizational and managerial tasks. The converse is also true: problems from
marketing and sales are part of organizational and managerial tasks.

\textbf{The model of a functioning system (MFS) is determined in two stages:}
...
dissatisfaction with salespeople.  The new system is promoted by the head of
the sales department, so between the head sales department and managers, we
also observe a conflict of interest. Further on the diagram show this
connection. \textbf{The core of the circuit always represents the minimum
  circuit.  conflict given by inventive situation.}
...
Fig. 6 he is designated as ROP). And maybe customers, which is unacceptable to
the company. In any case, customers should be plotted, since they ultimately
reflect the effect of internal changes in the sales department.

Why is a sales system created? To increase conversion rates, efficiency of
work with the client, and as a result - to conclude more transactions on large
amounts without hiring additional sales staff. What else “Hurt” the sales
system? CRM system implemented in the company (Customer Relationship
...
department - CRM system. Therefore, the diagram depicts a conflict a new sales
system with an existing CRM system, to which specialists of the department
sales managed to get used to. The problem is not that the installed system
does not have certain options, this is a secondary task, smaller than the one
we trying to formalize. The problem is that the “relationship“ of sellers with
new requirements of the CRM system.

In addition, the work of the sales department relates to the activities of
other company services - production, warehouse, logistics, accounting ... This
is important, but at the stage of setting the task it is too early to engage
in deep detailing of these processes; therefore, we denote “points touch
“sales department with other company services as“ end-to-end business
processes “, which can also be transformed under the action of the new sales
models. So we got a formed \textbf{model of a functioning system} consisting
of: sales staff, head of sales, the existing sales system, the new sales
system that is replacing it, target client groups, CRM systems and end-to-end
business processes:

\addpicture{Fig. 6. A model of a functioning system, presented in the form of
  a diagram. Abbr.: ROP - Sales Manager.}

\subsubsection*{The concept of a layer in a circuit.}
...
located on the lower layers in the context of the task. And although the
system in in most cases, it is defined as a set of interconnected elements, it
is obvious that the elements can have different hierarchical positions in the
studied system. The concept of a layer allows us to consider the elements of
the system, taking into account their hierarchy, where the hierarchy is
...
not at all the same as we depict in system operator.  The usual story is that
the subsystem controls the supersystem as for example, in fig. 6 seller
actions drive customer response, although customers typical supersystem. Does
a system operator allow you to show this seemingly commonplace situation? From
the perspective of this task, it is critically important! That is, \emph{if
  subsystems, supersystems, etc. quite rigidly fixed, the layers are
